...
set_config_method() {
 ...
 ${Tip} salsa20/chacha20-*系列加密方式，需要额外安装依赖 libsodium ，否则会无法启动ShadowsocksR !" && echo
	read -erp "(默认: 5. aes-128-ctr):" method
	[[ -z "${method}" ]] && method="5"
	if [[ ${method} == "1" ]]; then
		method="none"
	elif [[ ${method} == "2" ]]; then
...
set_config_protocol() {
 ...
 ${Green_font_prefix}5.${Font_color_suffix} auth_chain_a" && echo
	read -erp "(默认: 2. auth_sha1_v4):" protocol
...
set_config_obfs() {
 ...
 ${Green_font_prefix}5.${Font_color_suffix} tls1.2_ticket_auth" && echo
	read -erp "(默认: 5. tls1.2_ticket_auth):" obfs
	[[ -z "${obfs}" ]] && obfs="5"
	if [[ ${obfs} == "1" ]]; then
		obfs="plain"
	elif [[ ${obfs} == "2" ]]; then
...
Set_server_port() {
	while true; do
		...
		read -erp "(默认: 8888):" server_port_s
...
Set_crontab() {
 ...
 ${Green_font_prefix}9.${Font_color_suffix} 自定义输入" && echo
	read -erp "(默认: 2. 10分钟):" Crontab_time
...
Test() {
	...
	cd ${SSR_folder} || exit
	local_port=$(rand)
	for ((integer = 1; integer <= "${Like_num}"; integer++)); do
...
Download_SSRStatus() {
	cd "/usr/local" || exit 1
	wget -N --no-check-certificate "https://github.com/jumploop/SSRStatus/archive/master.zip"
...
Update_Shell() {
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/jumploop/doubi/master/ssrstatus.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	wget -N --no-check-certificate "https://raw.githubusercontent.com/jumploop/doubi/master/ssrstatus.sh" && chmod +x ssrstatus.sh
