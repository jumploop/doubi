...
sethttp() {
	...
	#设置目录
	echo "请输入要开放的目录(绝对路径)"
	read -erp "(直接回车, 默认当前文件夹):" httpfile
	if [[ -n $httpfile ]]; then
	...
	echo "========================"
	echo
	read -erp "按任意键继续，如有错误，请使用 Ctrl + C 退出." var
}
iptables_add() {
...
starthttp() {
	...
	[[ -n $PID ]] && echo -e "\033[41;37m [错误] \033[0m SimpleHTTPServer 正着运行，请检查 !" && exit 1
	sethttp
	iptables_add
	cd "${httpfile}" || exit
