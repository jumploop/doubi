...
ssr_log_file="/var/log/ssserver.log"
...
BBR_installation_status() {
  if [[ ! -e ${BBR_file} ]]; then
    ...
    cd "${file}" || exit
    if ! wget -N --no-check-certificate https://${github}/doubi/master/bbr.sh; then
...
ssr_link_qr() {
  SSRprotocol="${protocol//_compatible/ }"
  SSRobfs="${obfs//_compatible/ }"
...
View_User() {
  ...
  Get_IP
  Get_User
  now_mode=$(grep '"port_password"' "${config_user_file}")
 ...
 协议和混淆后面的[ _compatible ]，指的是 兼容原版协议/混淆。"
    ...
    for ((integer = user_total; integer >= 1; integer--)); do
...
Set_config_protocol_param() {
  while true; do
    ...
    if echo $((ssr_protocol_param + 0)) &>/dev/null; then
...
Set_config_speed_limit_per_con() {
  while true; do
    ...
    if echo $((ssr_speed_limit_per_con + 0)) &>/dev/null; then
...
Set_config_speed_limit_per_user() {
  while true; do
    ...
    if echo $((ssr_speed_limit_per_user + 0)) &>/dev/null; then
...
# 修改 配置信息
Modify_config_port() {
  sed -i 's/"server_port": '"${port}"'/"server_port": '"${ssr_port}"'/g' ${config_user_file}
}
Modify_config_password() {
  sed -i 's/"password": "'"${password}"'"/"password": "'"${ssr_password}"'"/g' ${config_user_file}
}
Modify_config_method() {
  sed -i 's/"method": "'"${method}"'"/"method": "'"${ssr_method}"'"/g' ${config_user_file}
}
Modify_config_protocol() {
  sed -i 's/"protocol": "'"${protocol}"'"/"protocol": "'"${ssr_protocol}"'"/g' ${config_user_file}
}
Modify_config_obfs() {
  sed -i 's/"obfs": "'"${obfs}"'"/"obfs": "'"${ssr_obfs}"'"/g' ${config_user_file}
}
Modify_config_protocol_param() {
  sed -i 's/"protocol_param": "'"${protocol_param}"'"/"protocol_param": "'"${ssr_protocol_param}"'"/g' ${config_user_file}
}
Modify_config_speed_limit_per_con() {
  sed -i 's/"speed_limit_per_con": '"${speed_limit_per_con}"'/"speed_limit_per_con": '"${ssr_speed_limit_per_con}"'/g' ${config_user_file}
}
Modify_config_speed_limit_per_user() {
  sed -i 's/"speed_limit_per_user": '"${speed_limit_per_user}"'/"speed_limit_per_user": '"${ssr_speed_limit_per_user}"'/g' ${config_user_file}
}
Modify_config_connect_verbose_info() {
  sed -i 's/"connect_verbose_info": '"${connect_verbose_info}"'/"connect_verbose_info": '"${ssr_connect_verbose_info}"'/g' ${config_user_file}
}
Modify_config_all() {
  Modify_config_port
  Modify_config_password
...
Modify_config_port_many() {
  sed -i 's/"'"${port}"'":/"'"${ssr_port}"'":/g' ${config_user_file}
}
Modify_config_password_many() {
  sed -i 's/"'"${password}"'"/"'"${ssr_password}"'"/g' ${config_user_file}
}
# 写入 配置信息
Write_configuration() {
...
Install_Libsodium() {
  ...
  if [[ ${release} == "centos" ]]; then
    ...
    echo -e "${Info} 解压..."
    tar -xzf libsodium-"${Libsodiumr_ver}".tar.gz && cd libsodium-"${Libsodiumr_ver}" || exit
    echo -e "${Info} 编译安装..."
...
View_user_connection_info() {
 ...
 ${Green_font_prefix}2.${Font_color_suffix} 显示 IP+IP归属地 格式" && echo
  read -r -e -p "(默认: 1):" ssr_connection_info
...
Modify_multi_port_user() {
  ...
  read -r -e -p "(默认: 取消):" modify_user_port
  ...
  if [[ -n "${del_user}" ]]; then
    ...
    Set_config_port
    Set_config_password
    sed -i "s/\"${port}\":\"${password}\"/\"${ssr_port}\":\"${ssr_password}\"/g" ${config_user_file}
    Del_iptables
    Add_iptables
    Save_iptables
...
Del_multi_port_user() {
  ...
  echo -e "请输入要删除的用户端口"
  read -r -e -p "(默认: 取消):" del_user_port
  ...
  if [[ -n ${del_user} ]]; then
    ...
    Del_iptables
    Save_iptables
    del_user_determine=${del_user:((${#del_user} - 1))}
...
————————
 ...
 注意： 锐速和LotServer不能同时安装/启动！" && echo
  read -r -e -p "(默认: 取消):" server_speeder_num
...
Install_ServerSpeeder() {
  ...
  cd /root || exit
  #借用91yun.rog的开心版锐速
  wget -N --no-check-certificate https://${github}/serverspeeder/master/serverspeeder.sh
  [[ ! -e "serverspeeder.sh" ]] && echo -e "${Error} 锐速安装脚本下载失败 !" && exit 1
  bash serverspeeder.sh
  sleep 2s
  PID=$(ps -ef | pgrep -f "serverspeeder.sh")
  if [[ -n ${PID} ]]; then
...
————————
 ...
 注意： 锐速和LotServer不能同时安装/启动！" && echo
  read -r -e -p "(默认: 取消):" lotserver_num
...
Uninstall_LotServer() {
  ...
  [[ -z ${unyn} ]] && echo && echo "已取消..." && exit 1
  if [[ ${unyn} == [Yy] ]]; then
    wget --no-check-certificate -qO /tmp/appex.sh "https://${github}/serverSpeeder_Install/master/appex.sh" && bash /tmp/appex.sh 'uninstall'
...
# 封禁 BT PT SPAM
BanBTPTSPAM() {
  wget -N --no-check-certificate https://${github}/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
  rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM() {
  wget -N --no-check-certificate https://${github}/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
  rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info() {
  SSR_installation_status
  ...
  if [[ ${connect_verbose_info} = "0" ]]; then
    ...
    read -r -e -p "(默认: n):" connect_verbose_info_ny
...
————————————
 ...
 ${Green_font_prefix}16.${Font_color_suffix} 安装内核并开启bbr加速
 "
menu_status
echo && read -r -e -p "请输入数字 [1-16]：" num
case "$num" in
1)
  Install_SSR
  ;;
