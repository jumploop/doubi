...
sh_ver="1.4.0"
github="raw.githubusercontent.com/jumploop/Linux-NetSpeed/master"
...
installbbr() {
	...
	stty erase '^H' && read -r -p "需要重启VPS后，才能开启BBR/BBR魔改版，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
installbbrplus() {
	...
	stty erase '^H' && read -r -p "需要重启VPS后，才能开启BBRplus，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
installlot() {
	...
	stty erase '^H' && read -r -p "需要重启VPS后，才能开启Lotserver，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
startbbrmod() {
	...
	if [[ "${release}" == "centos" ]]; then
		...
		mkdir bbrmod && cd bbrmod || exit
		wget -N --no-check-certificate http://${github}/bbr/tcp_tsunami.c
		echo "obj-m:=tcp_tsunami.o" >Makefile
		make -C /lib/modules/"${kernel_release}"/build M="$(pwd)" modules CC=/usr/bin/gcc
		chmod +x ./tcp_tsunami.ko
		cp -rf ./tcp_tsunami.ko /lib/modules/"${kernel_release}"/kernel/net/ipv4
		...
		make -C /lib/modules/"${kernel_release}"/build M="$(pwd)" modules CC=/usr/bin/gcc-4.9
		install tcp_tsunami.ko /lib/modules/"${kernel_release}"/kernel
		cp -rf ./tcp_tsunami.ko /lib/modules/"${kernel_release}"/kernel/net/ipv4
		depmod -a
	fi
...
startbbrmod_nanqinlang() {
	...
	if [[ "${release}" == "centos" ]]; then
		...
		cp -rf ./tcp_nanqinlang.ko /lib/modules/"${kernel_release}"/kernel/net/ipv4
		...
		echo "obj-m := tcp_nanqinlang.o" >Makefile
		make -C /lib/modules/"${kernel_release}"/build M="$(pwd)" modules CC=/usr/bin/gcc-4.9
		install tcp_nanqinlang.ko /lib/modules/"${kernel_release}"/kernel
		cp -rf ./tcp_nanqinlang.ko /lib/modules/"${kernel_release}"/kernel/net/ipv4
		depmod -a
	fi
...
remove_all() {
	...
	if [[ -e /appex/bin/lotServer.sh ]]; then
		bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/chiakge/lotServer/master/Install.sh) uninstall
	fi
	clear
	echo -e "${Info}:清除加速完成。"
	sleep 1s
...
*               hard    nofile          1000000" >/etc/security/limits.conf
	...
	read -r -p "需要重启VPS后，才能生效系统优化配置，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
————————————————————————————————" && echo
	...
	if [[ ${kernel_status} == "noinstall" ]]; then
		...
		echo -e " 当前状态: ${Green_font_prefix}已安装${Font_color_suffix} ${Green_font_prefix}${kernel_status}${Font_color_suffix} 加速内核 , ${Green_font_prefix}${run_status}${Font_color_suffix}"

	fi
	echo
	read -r -p " 请输入数字 [0-11]:" num
	case "$num" in
	0)
		Update_Shell
		;;
...
detele_kernel() {
	if [[ "${release}" == "centos" ]]; then
		rpm_total=$(rpm -qa | grep -v "${kernel_version}" | grep -v "noarch" | grep -c kernel)
		if [ "${rpm_total}" ] >"1"; then
		...
		deb_total=$(dpkg -l | awk '{print $2}' | grep -v "${kernel_version}" | grep -c linux-image)
		if [ "${deb_total}" ] >"1"; then
