...
check_threshold_centos() {
	for ((integer = port_num; integer >= 1; integer--)); do
		...
		[[ ${ip_num} -ge ${IP_threshold} ]] && echo -e " 端口: ${Red_font_prefix}${port_check}${Font_color_suffix} ,IP总数: ${Red_font_prefix}${ip_num}${Font_color_suffix} ,IP: ${Sky_blue_font_prefix}${ip_check}${Font_color_suffix}"
		ip_check=""
	done
}
...
get_IP_address() {
	...
	if [[ ${ip_num} -ge ${IP_threshold} ]]; then
		if [[ -n ${ip_check_1} ]]; then
			#echo "ip_num=${ip_num}"
			for ((integer_1 = ip_num; integer_1 >= 1; integer_1--)); do
