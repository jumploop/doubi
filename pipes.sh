...
check_installed_status() {
	[[ ! -e ${pipes_file} ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} PipeSocks 没有安装，请检查 !" && exit 1
}
check_new_ver() {
	...
	[[ -z ${pipes_new_ver} ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} PipeSocks 最新版本获取失败 !" && exit 1
}
check_ver_comparison() {
	pipes_now_ver=$(cat ${pipes_ver})
	if [[ ${pipes_now_ver} != "" ]]; then
		if [[ ${pipes_now_ver} != "${pipes_new_ver}" ]]; then
			echo -e "${Green_font_prefix}[信息]${Font_color_suffix} 发现 PipeSocks 已有新版本 [v${pipes_new_ver}] !"
			read -erp "是否更新 ? [Y/n] :" yn
			[[ -z "${yn}" ]] && yn="y"
			if [[ $yn == [Yy] ]]; then
				PID=$(ps -ef | grep "pipesocks" | grep -v "grep" | awk '{print $2}') && [[ -n $PID ]] && kill -9 "${PID}"
				Download_pipes
				Read_config
				Start_pipes
			fi
		else
			echo -e "${Green_font_prefix}[信息]${Font_color_suffix} 当前 PipeSocks 已是最新版本 [v${pipes_new_ver}] !" && exit 1
		fi
	else
...
Download_pipes() {
	...
	if [[ ${bit} == "x86_64" ]]; then
		...
		echo -e "${Red_font_prefix}[错误]${Font_color_suffix} 不支持 ${bit} !" && exit 1
	fi
	[[ ! -e "pipesocks-linux.tar.xz" ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} PipeSocks 下载失败 !" && exit 1
	[[ -e ${pipes_file} ]] && rm -rf ${pipes_file}
	tar -xJf pipesocks-linux.tar.xz && rm -rf pipesocks-linux.tar.xz
	[[ ! -e ${pipes_file} ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} PipeSocks 解压失败或压缩文件不完整 !" && exit 1
	mv pipesocks pipes
	mkdir pipesocks/
...
Read_config() {
	[[ ! -e ${pipes_config} ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} PipeSocks 配置文件不存在 !" && exit 1
...
Set_user_pipes() {
	...
	read -erp "(默认: doub.io):" pipes_passwd
	[[ -z "${pipes_passwd}" ]] && pipes_passwd="doub.io"
	echo && echo "————————————————————"
	echo -e "	密码 : ${Green_font_prefix}${pipes_passwd}${Font_color_suffix}"
	echo "————————————————————" && echo
}
Set_pipes() {
	check_installed_status
...
View_pipes() {
	...
	echo -e " 端口\t: ${Green_font_prefix}${pump_port}${Font_color_suffix}"
	echo -e " 密码\t: ${Green_font_prefix}${pump_passwd}${Font_color_suffix}"
	echo && echo "————————————————"
	PID=$(ps -ef | grep "pipesocks" | grep -v "grep" | awk '{print $2}')
	if [[ -n $PID ]]; then
		echo -e " 当前状态: ${Green_font_prefix}正在运行${Font_color_suffix}"
	else
		echo -e " 当前状态: ${Red_font_prefix}没有运行${Font_color_suffix}"
	fi
	echo
}
Install_pipes() {
	[[ -e ${pipes_file} ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} 检测到 PipeSocks 已安装，如需继续，请先卸载 !" && exit 1
	check_new_ver
	Set_user_pipes
	Download_pipes
	Service_pipes
...
Start_pipes() {
	check_installed_status
	PID=$(ps -ef | grep "pipesocks" | grep -v "grep" | awk '{print $2}')
	[[ -n $PID ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} PipeSocks 进程正在运行，请检查 !" && exit 1
	/etc/init.d/pipes start
}
Stop_pipes() {
	check_installed_status
	PID=$(ps -ef | grep "pipesocks" | grep -v "grep" | awk '{print $2}')
	[[ -z $PID ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} 没有发现 PipeSocks 进程运行，请检查 !" && exit 1
...
Log_pipes() {
	check_installed_status
	[[ ! -e ${pipes_log} ]] && echo -e "${Red_font_prefix}[错误]${Font_color_suffix} PipeSocks 日志文件不存在 !" && exit 1
	echo && echo -e "使用 ${Green_background_prefix} Ctrl+C ${Font_color_suffix} 键退出查看日志 !" && echo -e "如果需要查看完整日志内容，请用 ${Green_background_prefix}cat ${pipes_log}${Font_color_suffix} 命令。" && echo
	tail -f ${pipes_log}
}
Uninstall_pipes() {
	check_installed_status
